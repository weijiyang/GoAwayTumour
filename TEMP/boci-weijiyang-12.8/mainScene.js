...
var MainLayer = cc.Layer.extend({
    ...
    ctor : function()
    {
        ...
        var DropBlood=new dropBlood();
        this.addChild(DropBlood);
        DropBlood.setScale(0.5);
        DropBlood.x=size.width/2;
        DropBlood.y=size.height/2;
        DropBlood.setLocalZOrder(100);
        DropBlood.dropBloodAction();
...
*/
            ...
            // this.schedule(this.createMonster1,1);
        ...
        // 给被保护细胞添加血条
        var CellBlood=new dropBlood();
        protect.addChild(CellBlood);
        CellBlood.setScale(0.5);
        CellBlood.x=50;
        CellBlood.y=120;
        CellBlood.dropBloodAction();
...
/*        this.createTower();
        ...
        this.schedule(this.collision,1);
    },
    ...
    createMonster:function(dt){
        ...
        cc.log(this.Monster.length);
        if(this.Monster.length>=10){
            this.unschedule(this.createMonster);
            // this.schedule(this.createMonster2,1);
        }

    },
    createMonster2:function(dt){
        //实例化一个新的精灵对象
        ...
        cc.log(this.Monster.length);
        if(this.Monster.length>=10){
            this.unschedule(this.createMonster2);
            // this.schedule(this.createMonster1);
        }
    ...
    collision:function () {
        ...
        for(var i in that.Monster){
                if(cc.rectContainsPoint(this.pro.getBoundingBox(),that.Monster[i].getPosition())) {
                    that.Monster[i].getParent().removeChild(that.Monster[i]);
                    that.Monster.shift();
                    cc.log("碰到了"+that.Monster.length);
                    if(that.Monster.length==0){
                        this.MonsterTimes+=1;
                        cc.log("波次："+this.MonsterTimes);
                    }
                }
        }
    ...
    loadTiledMapRectArrayMap : function(){
        ...
        var tileGID =null;
        var properties =null;
        /*      cc.log(mapSize.height);
         cc.log(mapSize.width);*/
        for (var i = 0; i < mapSize.width; i++) {
            ...
            for (var j = 0; j < mapSize.height; j++) {
                ...
                // cc.log(properties.type);
                /*                if((i == 3)&&(j==5)){
                 bgLayer.getTileAt(cc.p(i,j)).setVisible(false);
                 cc.log("xx"+properties.type);
                 }*/
    ...
    initTc : function()
    {
        ...
        var continueItem = new cc.MenuItemImage(res.ContinueNormal_png,res.ContinueSelected_png, function () {
            ...
            this.pus.hidden(this.pus, function(){
                ...
                // cc.director.runScene(new LevelScene());
                if(this.pauseToggle.getSelectedIndex()==0){
                    cc.director.resume();
                }
                else{
                    cc.director.pause();
